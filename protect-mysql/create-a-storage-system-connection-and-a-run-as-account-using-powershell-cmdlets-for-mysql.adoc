...
summary: 'PowerShell cmdlet을 사용하여 MySQL 데이터베이스를 백업, 복원 또는 복제하려면 먼저 SVM(스토리지 가상 머신) 연결과 자격 증명을 만들어야 합니다.' 
---
= PowerShell cmdlet을 사용하여 MySQL용 스토리지 시스템 연결 및 자격 증명 만들기
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
PowerShell cmdlet을 사용하여 MySQL 데이터베이스를 백업, 복원 또는 복제하려면 먼저 SVM(스토리지 가상 머신) 연결과 자격 증명을 만들어야 합니다.
...
. SQLLIB의 경로를 설정합니다.
+
Windows의 경우 MySQL 플러그인은 SQLLIB 폴더의 기본 경로인 "C:\Program Files\IBM\SQLLIB\BIN"을 사용합니다.
