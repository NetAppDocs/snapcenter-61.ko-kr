...
summary: SnapCenter 에서 작업을 수행하기 위해 ONTAP 관리자 역할을 사용하지 않아도 되도록 ASA r2 시스템에 대한 ONTAP 클러스터 역할을 만들어야 합니다. 
---
...
[role="lead"]
SnapCenter 에서 작업을 수행하기 위해 ONTAP 관리자 역할을 사용하지 않아도 되도록 최소한의 권한이 있는 ONTAP 클러스터 역할을 만들어야 합니다.  여러 ONTAP CLI 명령을 실행하여 ONTAP 클러스터 역할을 만들고 최소 권한을 할당할 수 있습니다.
