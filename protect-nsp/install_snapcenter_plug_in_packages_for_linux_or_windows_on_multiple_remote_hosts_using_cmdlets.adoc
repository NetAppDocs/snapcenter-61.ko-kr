...
---
= cmdlet을 사용하여 여러 원격 호스트에 Linux, Windows 또는 AIX용 SnapCenter 플러그인 패키지 설치
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Install-SmHostPackage PowerShell cmdlet을 사용하여 Linux, Windows 또는 AIX용 SnapCenter 플러그인 패키지를 여러 호스트에 동시에 설치할 수 있습니다.

.시작하기 전에
호스트를 추가하는 사용자는 해당 호스트에 대한 관리 권한이 있어야 합니다.
